...
export default function ComparisonTable({
  ...
  const rows: {
    label: string;
    getValue: (t: Tool) => string | boolean | number;
  }[] = [
    ...
    { label: "Founded", getValue: (t) => t.founded },
  ];
          ...
          <tr className="border-b border-white/10 bg-[#111]">
            ...
            {tools.map((tool, i) => (
              <th
                ...
                style={{ borderTop: i === 0 ? `2px solid ${accentColor}` : undefined }}
              >
          ...
          {rows.map((row, rowIdx) => (
            <tr
              ...
              className={`border-b border-white/5 ${
                rowIdx % 2 === 0 ? "bg-[#0a0a0a]" : "bg-[#111]/50"
              }`}
            >
              <td className="p-4 text-white/50">{row.label}</td>
              {tools.map((tool) => (
                <td key={tool.id} className="p-4">
                  <Cell value={row.getValue(tool)} />
                </td>
              ))}
            </tr>
          ))}
          <tr className="bg-[#111]">
            <td className="p-4 text-white/50">Get Started</td>
            {tools.map((tool) => (
              <td key={tool.id} className="p-4">
                <a
                  ...
                  className="inline-block rounded-lg px-4 py-2 text-xs font-medium text-white transition-opacity hover:opacity-90"
