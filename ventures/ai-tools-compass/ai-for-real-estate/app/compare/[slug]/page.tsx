...
import { getComparisonBySlug, getAllSlugs, realEstateComparisons } from "@/data/comparisons";
import { getToolById, realEstateTools } from "@/data/tools";
...
export default function ComparisonPage({ params }: Props) {
  ...
  const articleSchema = {
    ...
    publisher: {
      "@type": "Organization",
      name: "AI Tools Compass",
    },
  };

  const relatedComparisons = realEstateComparisons
  ...
  return (
    <>
        ...
        <div className="mx-auto max-w-4xl px-4">
          ...
          <div className="mt-6 flex flex-wrap gap-3 text-sm">
            <span className="flex items-center gap-1.5 rounded-full bg-white/5 px-3 py-1.5 text-white/60">
              ...
              {comparison.toolIds.length} tools compared
            </span>
            <span className="flex items-center gap-1.5 rounded-full bg-white/5 px-3 py-1.5 text-white/60">
              <svg className="h-3.5 w-3.5 text-green-400" fill="currentColor" viewBox="0 0 20 20">
                <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" />
              </svg>
              {comparison.faqs.length} FAQs answered
            </span>
          </div>
        </div>
      </section>
      ...
      <div className="mx-auto max-w-4xl px-4 py-12">
        ...
        {winner && (
          <div className="mb-10 rounded-2xl border border-[#059669]/30 bg-[#059669]/5 p-6">
            <p className="mb-4 text-sm font-semibold uppercase tracking-wide text-[#059669]">
              Our Recommendation
            </p>
            <div className="grid gap-4 md:grid-cols-2">
              <div className="rounded-xl border border-green-500/20 bg-[#111] p-4">
                ...
                <a
                  ...
                  className="mt-3 inline-block rounded-lg bg-[#059669] px-4 py-2 text-sm font-medium text-white"
                >
                  Try {winner.name} →
                </a>
              </div>
        ...
        <div className="mb-12">
          ...
          {tools.length > 0 && (
            <ComparisonTable tools={tools as any} accentColor="#059669" />
          )}
        </div>
                ...
                >
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-4">
                      <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-[#059669]/20 text-base font-bold text-[#059669]">
                  ...
                  <div className="mt-5 flex gap-3">
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer nofollow"
                      className="rounded-lg bg-[#059669] px-5 py-2.5 text-sm font-medium text-white"
                    >
                      Try {tool.name} →
                    </a>
                    <a
              ...
              {comparison.buyingGuide.map((tip, i) => {
                ...
                return (
                  <div key={i} className="flex gap-4 rounded-xl border border-white/10 bg-[#111] p-4">
                    <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full bg-[#059669]/20 text-sm font-bold text-[#059669]">
                      {i + 1}
                    </div>
                    <div>
        ...
        <div className="mb-12">
          ...
          <div className="space-y-4">
            {comparison.faqs.map((faq, i) => (
              <details
                key={i}
                className="group rounded-xl border border-white/10 bg-[#111]"
              >
                <summary className="flex cursor-pointer items-center justify-between p-5 text-left font-medium text-white hover:text-[#059669]">
                  {faq.question}
                  <svg
                    className="h-5 w-5 flex-shrink-0 text-white/40 transition-transform group-open:rotate-180"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                  >
            ...
            <div className="grid gap-4 md:grid-cols-3">
              {relatedComparisons.map((c) => (
                <Link
                  key={c.slug}
                  href={`/compare/${c.slug}`}
                  className="rounded-xl border border-white/10 bg-[#111] p-4 transition-all hover:border-[#059669]/30"
                >
                  <p className="font-medium text-white hover:text-[#059669]">{c.title}</p>
        ...
        <div className="mt-10 rounded-xl border border-white/10 bg-[#111] p-4 text-xs text-white/40">
          ...
          Links on this page marked with an arrow (→) are affiliate links. We earn a commission
          if you sign up through these links. This doesn&apos;t affect our editorial rankings — we test
          tools independently and rank based on performance, not affiliate rates.
        </div>
      </div>
    </>
  );
